...
struct Block_literal_1 {
    ...
    int flags;   // 标志位，表示 block 一些附加信息
    int reserved;
    void (*invoke)(void *, ...);    // 函数指针
    struct Block_descriptor_1 {   // block 的附加描述信息，主要是 size 大小，以及 copy 和 dispose 函数的指针
...
// block 标志位
enum {
    BLOCK_HAS_COPY_DISPOSE =  (1 << 25),   // 包含 copy 和 dispose 方法
    BLOCK_HAS_CTOR =          (1 << 26), // helpers have C++ code
    BLOCK_IS_GLOBAL =         (1 << 28),   // block 是 global 类型
    BLOCK_HAS_STRET =         (1 << 29), // IFF BLOCK_HAS_SIGNATURE
    BLOCK_HAS_SIGNATURE =     (1 << 30),   // 有签名信息
};
typedef int BlockFlags;

// 获取 block 类型字符串（signature），signature 会包含参数和返回值信息
...
// 判断一个 block 是否是 RSSwizzleImpFactoryBlock 类型的 block
