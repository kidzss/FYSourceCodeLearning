...
// 把代理方法和对应的 block 属性链接起来，这样可以通过代理方法的选择子查找对应的 block。
+ (void)bk_linkProtocol:(Protocol *)protocol methods:(NSDictionary *)dictionary
{
	...
	[dictionary enumerateKeysAndObjectsUsingBlock:^(NSString *propertyName, NSString *selectorName, BOOL *stop) {
        ...
        // 这里的 propertyName 属性就是用户定义的 block
        // 用户定义 block 没有实现 getter 和 setter，使用了 @dynamic 关键词
        // 这里将动态实现 block 的 getter 和 setter（runtime 动态添加）
...
// 注册动态代理（dataSource、delegate）
// 参数传入 delegatename
// 根据 delegatename 替换 delegate 的 setter 和 getter
// 每次 get deledate 时，就会获取 A2DynamicDelegate 或其子类 A2DynamicDelegateXxx（真正的动态代理对象）
+ (void)bk_registerDynamicDelegateNamed:(NSString *)delegateName forProtocol:(Protocol *)protocol
{
    ...
    // 获取设置 delegate 的方法（setDelegate）
	SEL setter = setterForProperty(property, name);
    
    // 新的 setter SEL，加前缀 a_
	...
	if (![self instancesRespondToSelector:getter]) {
        ...
        // getter
        // 获取 delegte 时，获取的是 A2DynamicUIImagePickerControllerDelegate 对象
		IMP getterImplementation = imp_implementationWithBlock(^(NSObject *delegatingObject) {
